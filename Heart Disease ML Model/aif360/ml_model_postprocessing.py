...
def training_and_testing_model(df,index):
    ...
    df_train, df_test, X_train,X_test,y_train,y_test = train_test_split(df,X,y,test_size=0.2,random_state=index)
...
def validate(model,fair_pred,model_type,X,y,first=False):
    ...
    with open(f"./reports/postprocessing_models/aif360/heart_disease_metrics_report.txt",open_type) as f:
