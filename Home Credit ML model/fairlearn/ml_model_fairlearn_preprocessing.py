...
def training_model(dataset,index):
    ...
    svm_fair_model_pipeline = make_pipeline(StandardScaler(),LinearSVC(dual='auto',max_iter=1000))
    ...
    X_fair_train, X_fair_test, y_fair_train, y_fair_test = train_test_split(X_fair_selected,y_fair,test_size=0.2,random_state=index)
    ...
    for name,prediction in predictions.items():
        ...
        with open('./reports/fairness_reports/preprocessing/fairlearn/home_credit_report.txt',open_type) as f:
    ...
    pickle.dump(lr_fair_model_pipeline,open('./output_models/preprocessing_models/fairlearn/lr_home_credit_model.sav','wb'))
    pickle.dump(rf_fair_model_pipeline,open('./output_models/preprocessing_models/fairlearn/rf_home_credit_model.sav','wb'))
    pickle.dump(svm_fair_model_pipeline,open('./output_models/preprocessing_models/fairlearn/svm_home_credit_model.sav','wb'))
    pickle.dump(xgb_fair_model_pipeline,open('./output_models/preprocessing_models/fairlearn/xgb_home_credit_model.sav','wb'))
...
def print_time(time,index):
    ...
    with open('./reports/time_reports/fairlearn/home_credit_preprocessing_report.txt',open_type) as f:
