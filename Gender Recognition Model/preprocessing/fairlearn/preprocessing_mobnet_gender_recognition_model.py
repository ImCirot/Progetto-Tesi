...
def training_and_testing_model(std_df,fair_df):
    ...
    plt.ylabel('f1')
    plt.xlabel('epoch')
    plt.savefig('./figs/fairlearn/preprocessing_mobnet_f1.png')
    ...
    plt.ylabel('accuracy')
    plt.xlabel('epoch')
    plt.savefig('./figs/fairlearn/preprocessing_mobnet_accuracy.png')
    ...
    plt.xlabel('epoch')
    plt.savefig('./figs/fairlearn/preprocessing_mobnet_precision.png')
    ...
    plt.ylabel('recall')
    plt.xlabel('epoch')
    plt.savefig('./figs/fairlearn/preprocessing_mobnet_recall.png')

    mobnet_loss, mobnet_accuracy, mobnet_f1, mobnet_precision, mobnet_recall = mobnet_model.evaluate(fair_validaton_generator)

    with open('./reports/preprocessing_models/fairlearn/mobnet_gender_recognition_report.txt','w') as f:
    ...
    with open('./output_models/preprocessing_models/mobnet_model/fairlearn/mobnet_gender_recognition_model.json','w') as f:
        f.write(m_json)

    mobnet_model.save_weights('./output_models/preprocessing_models/mobnet_model/fairlearn/mobnet_std_weights.h5')
    ...
    pred = mobnet_model.predict(validaton_generator)
    print(pred)
    pred = np.argmax(pred,axis=1)
    print(pred)
    ...
    for name,value in predictions.items():
        ...
        with open('./reports/fairness_reports/preprocessing/fairlearn/mobnet_gender_report.txt',open_type) as f:
...
def print_time(time):
    with open('./reports/time_reports/gender/fairlearn/mobnet_preprocessing_report.txt','w') as f:
