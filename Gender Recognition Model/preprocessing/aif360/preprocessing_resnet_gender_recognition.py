...
def training_and_testing_model(std_df,fair_df):
    ...
    resnet_model.compile(optimizer="adam", loss='categorical_crossentropy', metrics=['accuracy',tfa.metrics.F1Score(num_classes=2),tf.keras.metrics.Precision(),tf.keras.metrics.Recall()])
    ...
    plt.ylabel('f1')
    plt.xlabel('epoch')
    plt.savefig('./figs/aif360/preprocessing_resnet_f1.png')
    ...
    plt.xlabel('epoch')
    plt.savefig('./figs/aif360/preprocessing_resnet_precision.png')
    ...
    plt.ylabel('recall')
    plt.xlabel('epoch')
    plt.savefig('./figs/aif360/preprocessing_resnet_recall.png')

    resnet_loss, resnet_accuracy, resnet_f1, resnet_precision, resnet_recall = resnet_model.evaluate(validation_generator)
    ...
    pred = resnet_model.predict(validation_generator)
    pred = np.argmax(pred,axis=1)
...
def print_time(time):
    with open('./reports/time_reports/gender/aif360/resnet_preprocessing_report.txt','w') as f:
