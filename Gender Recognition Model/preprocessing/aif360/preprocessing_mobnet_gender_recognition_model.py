...
def training_and_testing_model(std_df,fair_df):
    ...
    plt.ylabel('F1')
    plt.xlabel('epoch')
    plt.savefig('./figs/preprocessing_mobnet_f1.png')
    ...
    plt.ylabel('accuracy')
    plt.xlabel('epoch')
    plt.savefig('./figs/preprocessing_mobnet_accuracy.png')
    ...
    plt.xlabel('epoch')
    plt.savefig('./figs/preprocessing_mobnet_precision.png')
    ...
    plt.ylabel('recall')
    plt.xlabel('epoch')
    plt.savefig('./figs/preprocessing_mobnet_recall.png')

    mobnet_loss, mobnet_accuracy, mobnet_f1, mobnet_precision, mobnet_recall = mobnet_model.evaluate(validation_generator)

    with open('./reports/preprocessing_models/aif360/mobnet_gender_recognition_report.txt','w') as f:
        ...
        f.write(f"Recalll: {round(mobnet_recall,3)}\n")

    m_json = mobnet_model.to_json()
    with open('./output_models/preprocessing_models/mobnet_model/aif360/mobnet_gender_recognition_model.json','w') as f:
        f.write(m_json)

    mobnet_model.save_weights('./output_models/preprocessing_models/mobnet_model/aif360/mobnet_std_weights.h5')
    ...
    pred = mobnet_model.predict(validation_generator)
    pred = np.argmax(pred,axis=1)
...
def print_metrics(message,metric,first_message=False):
    ...
    with open('./reports/fairness_reports/preprocessing/aif360/mobnet_gender_report.txt',open_type) as f:
        f.write(f'{message}: {round(metric,3)}\n')

def print_time(time):
    with open('./reports/time_reports/gender/aif360/mobnet_preprocessing_report.txt','w') as f:
