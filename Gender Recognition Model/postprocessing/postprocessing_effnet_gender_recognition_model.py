...
def load_dataset():
    ...
    prediction = pd.read_csv('./reports/predictions/effnet_prediction.txt')
...
def postop_model(df,prediction):
    ...
    with open('./reports/postprocessing_models/effnet_gender_recognition_report.txt','w') as f:
...
def print_metrics(message,metric,first_message=False):
    ...
    with open('./reports/fairness_reports/postprocessing/effnet_gender_report.txt',open_type) as f:
        f.write(f'{message}: {round(metric,3)}\n')

def print_time(time):
    with open('./reports/time_reports/gender/effnet_postprocessing_report.txt','w') as f:
